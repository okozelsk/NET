...
{
    /// <summary>
    /// Exponential distribution parameters (unsigned)
    /// </summary>
    [Serializable]
    public class UExponentialDistrSettings : RCNetBaseSettings, IDistrSettings
    {
        ...
        /// </summary>
        public const string XsdTypeName = "UExponentialDistrCfgType";
        ...
        private void Check()
        {
            if (Mean <= 0)
            {
                throw new Exception($"Incorrect Mean ({Mean.ToString(CultureInfo.InvariantCulture)}) value. Mean must be GT 0.");
            }
            return;
        }
